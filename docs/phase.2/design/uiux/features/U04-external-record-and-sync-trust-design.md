# U04 External Record and Sync Trust - UI UX Detailed Design
...
1. Core components required for external record, sync log, conflict panel.
...
## Screen Blueprint
1. External metadata card: system, connector, external matter/contact links.
2. Field mapping table: local value, external value, source, status.
3. Conflict drawer: diff view with resolution actions.
4. Sync activity timeline: inbound/outbound events with retry.

## State Design
1. Healthy sync with no conflicts.
2. Conflict pending state with blocked publish-back.
3. Retry required state with connector diagnostics.
