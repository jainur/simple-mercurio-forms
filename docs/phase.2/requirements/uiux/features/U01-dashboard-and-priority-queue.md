# U01 Dashboard and Priority Queue - UI UX Detailed Requirements
...
1. The screen or workflow must support: priority queue, stage board, deadline widgets.
...
1. Dashboard must prioritize blockers, pending approvals, imminent filing deadlines, and sync failures.
2. Dashboard must support saved filters by role and assigned team.
3. Matters-by-stage view must allow one-click drilldown to matter workspace.
4. Priority queue items must show owner, due date, and next action.

## User Journey Requirements
1. Assistant opens dashboard and resolves top blocked item in at most two clicks.
2. Lawyer opens dashboard and accesses pending approvals without operational noise.
3. Admin opens dashboard and sees sync failure clusters requiring intervention.
