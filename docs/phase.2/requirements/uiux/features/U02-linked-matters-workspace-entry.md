# U02 Linked Matters Workspace Entry - UI UX Detailed Requirements
...
1. The screen or workflow must support: linked matter list, filters, status chips.
...
1. Linked Matters list must show external PMS badge, matter ID, and sync health.
2. Search and filters must include procedure type, stage, assignee, and sync state.
3. Workspace entry must preserve context from selected matter.
4. Unlinked or degraded sync states must display guided recovery actions.

## User Journey Requirements
1. Assistant starts in linked matters, filters to assigned cases, and opens target workspace.
2. Admin inspects sync health and identifies matters requiring remap.
