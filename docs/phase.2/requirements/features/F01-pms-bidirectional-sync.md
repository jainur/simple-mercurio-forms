# F01 PMS Bidirectional Sync - Detailed Requirements

## Objective
Synchronize matters, statuses, and artifacts with external PMS systems

## In Scope
- Phase 2 implementation for PMS Bidirectional Sync
...
1. The system must implement PMS Bidirectional Sync flows aligned to Phase 2 state progression.
2. The system must expose stable APIs or integration contracts for: /cases webhooks adapters.
...
1. Support webhook and polling integration modes for PMS providers.
2. Map case, contact, timeline, and artifact metadata bidirectionally.
3. Support retries and dedupe keys for outbound status updates.
4. Expose sync health and last-sync timestamps per case.
