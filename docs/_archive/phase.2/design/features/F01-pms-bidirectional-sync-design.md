# F01 PMS Bidirectional Sync - Detailed Design

## Design Goals
- Implement Synchronize matters, statuses, and artifacts with external PMS systems with deterministic contracts and auditable behavior.
- Minimize coupling with adjacent features.
- Keep interfaces stable for iterative delivery.

## Components
1. API/interface boundary for /cases webhooks adapters
...
- Primary state context: CASE_CREATED
...
- Primary interfaces: /cases webhooks adapters
- State anchor: CASE_CREATED
- Ensure contracts are traceable to requirements ID F01.

## Concrete Design
- Adapter components: webhook receiver, polling worker, schema mapper, outbound sync publisher.
- Data entities: pms_sync_state, sync_cursor, sync_error_log.
- API contracts: inbound webhook auth and signature verification, outbound retry queue with dedupe key.
- State impact: timeline updates can trigger readiness and monitoring notifications.
