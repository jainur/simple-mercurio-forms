# F04 Eligibility and Legal Grounding (GraphRAG) - Detailed Design

## Design Goals
- Implement Produce eligibility matrix with citations and evidence gaps with deterministic contracts and auditable behavior.
- Minimize coupling with adjacent features.
- Keep interfaces stable for iterative delivery.

## Components
1. API/interface boundary for /reasoning /eligibility
...
- Primary state context: ELIGIBILITY_ASSESSMENT_READY
...
- Primary interfaces: /reasoning /eligibility
- State anchor: ELIGIBILITY_ASSESSMENT_READY
- Ensure contracts are traceable to requirements ID F04.

## Concrete Design
- Components: eligibility worker, citation linker, gap recommender, comparison scorer.
- Data entities: eligibility_matrix_row, citation_ref, recommendation_item.
- API contracts: eligibility run endpoint and comparison endpoint.
- State impact: eligibility output gates document planning and readiness review.
