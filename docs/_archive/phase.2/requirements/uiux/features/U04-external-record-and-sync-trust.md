# U04 External Record and Sync Trust - UI UX Detailed Requirements

## Objective
Show source-of-record fields, sync status, and conflict resolution actions
...
1. The screen or workflow must support: external record, sync log, conflict panel.
...
1. External Record page must display per-field source, last sync timestamp, and conflict state.
2. Conflict resolution UI must support choose-local, choose-external, and merge where valid.
3. Sync log must include event direction, status, and retry controls.
4. Publish-back preview must show exactly which fields will be updated.

## User Journey Requirements
1. Assistant resolves a conflict and confirms publish-back safely.
2. Admin reviews sync failures and retries targeted events.
