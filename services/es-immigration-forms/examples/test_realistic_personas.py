...
from scripts.fill_form import fill_form
from scripts.forms_registry import get_mapper_function

TEXT_TYPES = {"Text", "ComboBox", "ListBox"}
OUT_DIR = ROOT / "data" / "forms" / "filled" / "realistic-personas"
...
def _load_definition(form_code: str) -> dict[str, Any]:
    return json.loads((ROOT / "data" / "forms" / "definitions" / f"{form_code}.json").read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    for idx, code in enumerate(codes):
        def_path = ROOT / "data" / "forms" / "definitions" / f"{code}.json"
